...
# SDFT prompts/optimizer-step. MUST divide the train-set size or the GRPO RepeatSampler drops the
# remainder each epoch (93 % 32 = 29 dropped; 93 = 3*31 keeps all 93). SFT's sampler keeps all rows.
NPPB=${NPPB:-31}
...
INSIGHT_PATH=${INSIGHT_PATH:-$HOME/arc-train93-split/insight_knowledge.jsonl}
